...
impl<'src, D> Recipe<'src, D> {
    ...
    pub(crate) fn run<'run>(
        ...
        if self.shebang {
            ...
            {
                let mut f =
                    fs::File::create(&path).map_err(|error| RuntimeError::TmpdirIoError {
                        recipe: self.name(),
                        io_error: error,
                    })?;
            ...
            let mut command =
                Platform::make_shebang_command(&path, &context.search.working_directory, shebang)
